...
char** cmdList(char *string)
{
  ...
  {
    numCmds++;
      printf("\nsubstring memloc = %p; string memloc = %p", substring, string);
...
//      printf("\n%c, %d", string[k], freezeRight);
  ...
  {
    return string;//?????????????????MAY CAUSE ERRORS L8R
  }
...
char* replace(char* big, char *small, int start)
{
  ...
  {
/*    if(i > start && big[i] != small[j])
    {
      printf("\nCALLED! %c vs %c", big[i], small[j]);
      printf("\noldbig = %s", big);
      big = parse(big, &dummy);
      printf(" newbig = %s", big);
      return big;
